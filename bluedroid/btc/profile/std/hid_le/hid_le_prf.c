...
yoc_bt_uuid_t char_info_uuid = {LEN_UUID_16, {CHAR_HID_INFO_UUID}};
yoc_bt_uuid_t char_ctnl_pt_uuid = {LEN_UUID_16, {CHAR_HID_CTNL_PT_UUID}};
yoc_bt_uuid_t char_report_map_uuid = {LEN_UUID_16, {CHAR_REPORT_MAP_UUID}};
yoc_bt_uuid_t char_report_uuid = {LEN_UUID_16, {CHAR_REPORT_UUID}};
yoc_bt_uuid_t char_proto_mode_uuid = {LEN_UUID_16, {CHAR_PROTOCOL_MODE_UUID}};
yoc_bt_uuid_t char_kb_in_report_uuid = {LEN_UUID_16, {CHAR_BOOT_KB_IN_REPORT_UUID}};
yoc_bt_uuid_t char_kb_out_report_uuid = {LEN_UUID_16, {CHAR_BOOT_KB_OUT_REPORT_UUID}};
yoc_bt_uuid_t char_mouse_in_report_uuid = {LEN_UUID_16, {CHAR_BOOT_MOUSE_IN_REPORT_UUID}};
...
**  Constants
*****************************************************************************/
static void hidd_le_profile_cb(yoc_gatts_evt_t event,  yoc_gatts_t *p_data);
...
static void hidd_add_characterisitc(const char_desc_t *char_desc)
{
    ...
    if (char_desc->char_uuid != 0x00) {
        ...
        yoc_ble_gatts_add_char (service_id,
                                char_desc->char_uuid,
                                char_desc->perm,
                                char_desc->prop);
    }
...
{
    yoc_gatts_rsp_t rsp;
    yoc_bt_uuid_t uuid = {LEN_UUID_16, {ATT_SVC_HID}};
    ...
    case YOC_GATTS_REG_EVT:
        ...
        if (p_data->reg_oper.status != YOC_GATT_OK) {
    ...
    case YOC_GATTS_CREATE_EVT:
        if (p_data->create.uuid.uu.uuid16 == ATT_SVC_HID) {
            ...
            yoc_ble_gatts_start_srvc(p_data->create.service_id);
        ...
        if (hid_char_idx <= HIDD_LE_CHAR_MAX) { //added the characteristic until the index overflow
            ...
            if ((p_data->add_result.char_uuid.uu.uuid16 == CHAR_BOOT_KB_IN_REPORT_UUID) ||
                ...
                BTC_TRACE_ERROR("p_data->add_result.char_uuid.uu.uuid16 = %x\n",
                          p_data->add_result.char_uuid.uu.uuid16);
                yoc_ble_gatts_add_char_descr (hidd_le_env.hidd_clcb.cur_srvc_id,
                                              GATT_PERM_WRITE,
                                              &uuid);
    ...
    case YOC_GATTS_WRITE_EVT:
        yoc_ble_gatts_send_rsp (p_data->req_data.conn_id, p_data->req_data.trans_id,
...
void hidd_le_create_service(BOOLEAN is_primary)
{
    ...
    yoc_ble_gatts_create_srvc (server_if, &uuid, inst, num_handle, is_primary);
}
...
void hidd_rsp (uint32_t trans_id, uint16_t conn_id, uint8_t app_id,
               yoc_gatt_status_t status, uint8_t event, tGATTS_DATA *p_rsp)
{
    hidd_inst_t *p_inst = &hidd_le_env.hidd_inst;
    tGATTS_RSP  rsp;
    uint8_t   *pp;
    ...
    if (p_inst->pending_evt == event) {
        ...
        case HIDD_LE_READ_INFO_EVT:
            ...
            yoc_ble_gatts_send_rsp(conn_id, trans_id, status, &rsp);
            break;
...
** Returns          NULL
*******************************************************************************/
yoc_gatt_status_t hidd_le_init (void)
{
    ...
    yoc_ble_gatts_app_register(&app_uuid, hidd_le_profile_cb);
