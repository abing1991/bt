    ...
    for (size_t i = 1; i <= BTA_JV_MAX_RFC_SR_SESSION; i++) {
        if (spp_local_param.spp_slots[i] == NULL) {
            ...
            if (spp_local_param.spp_mode == YOC_SPP_MODE_VFS) {
                ...
                if (yoc_vfs_register_fd(spp_local_param.spp_vfs_id, &spp_local_param.spp_slots[i]->fd) != YOC_OK) {
...
static void *btc_spp_rfcomm_inter_cb(tBTA_JV_EVT event, tBTA_JV *p_data, void *user_data)
{
    ...
    case BTA_JV_RFCOMM_SRV_OPEN_EVT:
        ...
        memcpy(slot->addr, p_data->rfc_srv_open.rem_bda, YOC_BD_ADDR_LEN);
        slot->connected = TRUE;
...
static void btc_spp_connect(btc_spp_args_t *arg)
{
    ...
    memcpy(slot->addr, arg->connect.peer_bd_addr, YOC_BD_ADDR_LEN);
...
void btc_spp_cb_handler(btc_msg_t *msg)
{
    ...
    case BTA_JV_RFCOMM_OPEN_EVT:
        ...
        memcpy(param.open.rem_bda, p_data->rfc_open.rem_bda, YOC_BD_ADDR_LEN);
    ...
    case BTA_JV_RFCOMM_SRV_OPEN_EVT:
        ...
        memcpy(param.srv_open.rem_bda, p_data->rfc_srv_open.rem_bda, YOC_BD_ADDR_LEN);
...
yoc_err_t btc_spp_vfs_register()
{
    yoc_vfs_t vfs = {
    ...
    // file descriptors
    if (yoc_vfs_register_with_id(&vfs, NULL, &spp_local_param.spp_vfs_id) != YOC_OK) {
        return YOC_FAIL;
    }
