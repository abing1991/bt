...
static void btc_bt_set_scan_mode(yoc_bt_scan_mode_t mode)
{
    tBTA_DM_DISC disc_mode;
    tBTA_DM_CONN conn_mode;

    switch (mode) {
    case YOC_BT_SCAN_MODE_NONE:
        disc_mode = BTA_DM_NON_DISC;
        conn_mode = BTA_DM_NON_CONN;
        break;

    case YOC_BT_SCAN_MODE_CONNECTABLE:
        disc_mode = BTA_DM_NON_DISC;
        conn_mode = BTA_DM_CONN;
        break;

    case YOC_BT_SCAN_MODE_CONNECTABLE_DISCOVERABLE:
    ...
    if ( p_dest_data->len > sizeof(tBTA_DM_SEARCH)){
        ...
        case BTA_DM_INQ_RES_EVT: {
            if (p_src_data->p_data->inq_res.p_eir) {
                ...
                memcpy(p_dest_data->p_data->inq_res.p_eir, p_src_data->p_data->inq_res.p_eir, HCI_EXT_INQ_RYOCONSE_LEN);
            }
        }
        break;
...
static void bte_search_devices_evt(tBTA_DM_SEARCH_EVT event, tBTA_DM_SEARCH *p_data)
{
    ...
    case BTA_DM_INQ_RES_EVT: {
        if (p_data->inq_res.p_eir) {
            param_len += HCI_EXT_INQ_RYOCONSE_LEN;
        }
    }
    break;
    ...
    case BTA_DM_INQ_RES_EVT: {
        ...
        do {
            ...
            if (p_data->p_data->inq_res.p_eir) {
                BTC_STORAGE_FILL_PROPERTY(&prop[2], YOC_BT_GAP_DEV_PROP_EIR, HCI_EXT_INQ_RYOCONSE_LEN, p_data->p_data->inq_res.p_eir);
                num_prop++;
            }
...
static void btc_gap_bt_search_services(char *p_param)
{
    ...
    case BTA_DM_DISC_RES_EVT: {
        ...
        if (p_data->p_data->disc_res.result == BTA_SUCCESS) {
            uuid_list = malloc(sizeof(yoc_bt_uuid_t) * p_data->p_data->disc_res.num_uuids);
            if (uuid_list) {
...
yoc_err_t btc_gap_bt_get_cod(yoc_bt_cod_t *cod)
{
    tBTA_UTL_COD p_cod;
...
yoc_err_t btc_gap_bt_remove_bond_device(btc_gap_bt_args_t *arg)
{
    BD_ADDR bd_addr;
