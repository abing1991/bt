...
static aos_task_t  btc_aa_snk_task_hdl;
static aos_queue_t btc_aa_snk_data_queue;
static aos_queue_t btc_aa_snk_ctrl_queue;
static osi_sem_t   btc_aa_snk_sem;
static char data_buffer[BTC_A2DP_SINK_DATA_QUEUE_LEN * sizeof(int32_t)];
static char ctrl_buffer[BTC_A2DP_SINK_CTRL_QUEUE_LEN * sizeof(void *)];
...
static void btc_a2dp_sink_ctrl_post(uint32_t sig, void *par)
{
    ...
    if (aos_queue_send(&btc_aa_snk_ctrl_queue, &evt, sizeof(BtTaskEvt_t *)) != 0) {
        APPL_TRACE_WARNING("btc_aa_snk_ctrl_queue failed, sig 0x%x\n", sig);
    } else {
        osi_sem_give(&btc_aa_snk_sem);
    }
}
...
static void btc_a2dp_sink_task_handler(void *arg)
{
    ...
    for (;;) {
        osi_sem_take(&btc_aa_snk_sem, AOS_WAIT_FOREVER);

        int32_t data_evt;

        if (aos_queue_recv(&btc_aa_snk_data_queue, 0, &data_evt, &len) == 0) {
        ...
        if (aos_queue_recv(&btc_aa_snk_ctrl_queue, 0, &e, &len) == 0) {
...
bool btc_a2dp_sink_startup(void)
{
    ...
    int ret = osi_sem_new(&btc_aa_snk_sem, 0, 0);
    aos_check_return_einval(!ret);

    ret = aos_queue_new(&btc_aa_snk_data_queue, data_buffer, BTC_A2DP_SINK_DATA_QUEUE_LEN * sizeof(int32_t), sizeof(int32_t));
    aos_check_return_einval(!ret);

    ret = aos_queue_new(&btc_aa_snk_ctrl_queue, ctrl_buffer, BTC_A2DP_SINK_CTRL_QUEUE_LEN * sizeof(void *), sizeof(void *));
    aos_check_return_einval(!ret);

    ret = aos_task_new_ext(&btc_aa_snk_task_hdl, BTC_A2DP_SINK_TASK_NAME, &btc_a2dp_sink_task_handler, NULL, BTC_A2DP_SINK_TASK_STACK_SIZE, BTC_A2DP_SINK_TASK_PRIO);
    aos_check_return_einval(!ret);
...
void btc_a2dp_sink_shutdown(void)
{
    ...
    //aos_task(btc_aa_snk_task_hdl);
    //btc_aa_snk_task_hdl = NULL;

    osi_sem_free(&btc_aa_snk_sem);

    aos_queue_free(&btc_aa_snk_data_queue);
    //btc_aa_snk_data_queue = NULL;

    aos_queue_free(&btc_aa_snk_ctrl_queue);
    //btc_aa_snk_ctrl_queue = NULL;

    //aos_queue_free(&btc_aa_snk_queue_set);
...
{
    if (aos_queue_send(&btc_aa_snk_data_queue, &data_type, 4) != 0) {
