...
static aos_task_t  btc_aa_src_task_hdl;
static aos_queue_t btc_aa_src_data_queue;
static aos_queue_t btc_aa_src_ctrl_queue;
static aos_sem_t   btc_aa_src_queue_set;

static char data_buffer[BTC_A2DP_SOURCE_DATA_QUEUE_LEN * sizeof(void*)];
static char ctrl_buffer[BTC_A2DP_SOURCE_CTRL_QUEUE_LEN * sizeof(void*)];
...
static void btc_a2dp_source_ctrl_post(uint32_t sig, void *par)
{
    ...
    if (aos_queue_send(&btc_aa_src_ctrl_queue, &evt, sizeof(BtTaskEvt_t*)) != 0) {
...
bool btc_a2dp_source_startup(void)
{
    ...
    int ret = osi_sem_new(&btc_aa_src_queue_set, 0, 0);
    aos_check_return_einval(!ret);

    //btc_aa_src_data_queue = xQueueCreate(BTC_A2DP_SOURCE_DATA_QUEUE_LEN, sizeof(void *));
    ret = aos_queue_new(&btc_aa_src_data_queue, data_buffer, BTC_A2DP_SOURCE_DATA_QUEUE_LEN * sizeof(int32_t), sizeof(int32_t));
    aos_check_return_einval(!ret);
    //xQueueAddToSet(btc_aa_src_data_queue, btc_aa_src_queue_set);


    ret = aos_queue_new(&btc_aa_src_ctrl_queue, ctrl_buffer, BTC_A2DP_SOURCE_CTRL_QUEUE_LEN * sizeof(int32_t), sizeof(int32_t));
    aos_check_return_einval(!ret);
    ...
    ret = aos_task_new_ext(&btc_aa_src_task_hdl, BTC_A2DP_SOURCE_TASK_NAME, btc_a2dp_source_task_handler, NULL, BTC_A2DP_SOURCE_TASK_STACK_SIZE, BTC_A2DP_SOURCE_TASK_PRIO);
    aos_check_return_einval(!ret);
...
void btc_a2dp_source_shutdown(void)
{
    ...
    aos_queue_free(&btc_aa_src_data_queue);

    aos_queue_free(&btc_aa_src_ctrl_queue);

    aos_sem_free(&btc_aa_src_queue_set);
...
{
    if (aos_queue_send(&btc_aa_src_data_queue, &data_type, 4) != 0) {
...
BOOLEAN btc_a2dp_source_stop_audio_req(void)
{
     ...
     */
    if (aos_queue_is_valid(&btc_aa_src_ctrl_queue)) {
