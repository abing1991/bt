...
static yoc_hf_client_incoming_data_cb_t btc_hf_client_incoming_data_cb = NULL;
static yoc_hf_client_outgoing_data_cb_t btc_hf_client_outgoing_data_cb = NULL;
...
if (! btc_hf_client_cb.initialized || \
    btc_hf_client_cb.state != YOC_HF_CLIENT_CONNECTION_STATE_SLC_CONNECTED) { \
    return BT_STATUS_NOT_READY; \
} \
} while (0)
...
{
    if (((btc_hf_client_cb.state == YOC_HF_CLIENT_CONNECTION_STATE_CONNECTED) ||
            (btc_hf_client_cb.state == YOC_HF_CLIENT_CONNECTION_STATE_SLC_CONNECTED))&&
...
static bt_status_t connect_int( bt_bdaddr_t *bd_addr, uint16_t uuid )
{
    ...
    btc_hf_client_cb.state = YOC_HF_CLIENT_CONNECTION_STATE_CONNECTING;
...
bt_status_t btc_hf_client_connect_audio( bt_bdaddr_t *bd_addr )
{
    ...
    if (is_connected(bd_addr))
    {
        ...
        do {
            ...
            memcpy(param.audio_stat.remote_bda, &btc_hf_client_cb.connected_bda, sizeof(yoc_bd_addr_t));
    ...
    switch (event)
    {
        ...
        case BTA_HF_CLIENT_OPEN_EVT:
            if (p_data->open.status == BTA_HF_CLIENT_SUCCESS)
            {
                ...
                btc_hf_client_cb.state = YOC_HF_CLIENT_CONNECTION_STATE_CONNECTED;
                btc_hf_client_cb.peer_feat = 0;
                btc_hf_client_cb.chld_feat = 0;
                //clear_phone_state();
            }
            else if (btc_hf_client_cb.state == YOC_HF_CLIENT_CONNECTION_STATE_CONNECTING)
            {
                btc_hf_client_cb.state = YOC_HF_CLIENT_CONNECTION_STATE_DISCONNECTED;
            }
            else
            {
            ...
            if (btc_hf_client_cb.state == YOC_HF_CLIENT_CONNECTION_STATE_DISCONNECTED)
        ...
        case BTA_HF_CLIENT_AT_RESULT_EVT:
            do {
                ...
                btc_hf_client_cb_to_app(YOC_HF_CLIENT_AT_RYOCONSE_EVT, &param);
            } while (0);
            break;
