...
static aos_task_t btc_task_handler;
static aos_queue_t btc_queue;
static int task_run;
static char queue_buf[BTC_TASK_QUEUE_LEN * sizeof(btc_msg_t)];
...
#if (GATTS_INCLUDED == TRUE && BLUIF_INCLUDED == TRUE)
...
static void btc_task(void *arg)
{
    ...
    while (task_run) {
        if (aos_queue_recv(&btc_queue, AOS_WAIT_FOREVER, &msg, &len) == 0) {
...
static bt_status_t btc_task_post(btc_msg_t *msg, task_post_t timeout)
{
    ...
    if (aos_queue_send(&btc_queue, msg, sizeof(btc_msg_t)) != 0) {
...
{
    int ret;
    ret = aos_queue_new(&btc_queue, queue_buf, BTC_TASK_QUEUE_LEN * sizeof(btc_msg_t), sizeof(btc_msg_t));
    ...
    task_run = 1;
    ret = aos_task_new_ext(&btc_task_handler, "Btc_task", btc_task, NULL, BTC_TASK_STACK_SIZE, BTC_TASK_PRIO);

    if (ret != 0){
        aos_queue_free(&btc_queue);
        return BT_STATUS_NOMEM;
    }
...
{
    int  wait_size = aos_queue_get_count(&btc_queue);
