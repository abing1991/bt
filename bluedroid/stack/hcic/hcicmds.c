...
void btsnd_hcic_write_ext_inquiry_response (void *buffer, UINT8 fec_req)
{
    ...
    p->len    = HCIC_PREAMBLE_SIZE + HCIC_PARAM_SIZE_EXT_INQ_RYOC;
    p->offset = 0;

    UINT16_TO_STREAM (pp, HCI_WRITE_EXT_INQ_RYOCONSE);
    UINT8_TO_STREAM  (pp, HCIC_PARAM_SIZE_EXT_INQ_RYOC);
                                     ...
                                     UINT8 oob_present, UINT8 auth_req)
{
    ...
    if ((p = HCI_GET_CMD_BUF(HCIC_PARAM_SIZE_IO_CAP_RYOC)) == NULL) {
        return (FALSE);
    }

    pp = (UINT8 *)(p + 1);

    p->len    = HCIC_PREAMBLE_SIZE + HCIC_PARAM_SIZE_IO_CAP_RYOC;
    p->offset = 0;

    UINT16_TO_STREAM (pp, HCI_IO_CAPABILITY_REQUEST_REPLY);
    UINT8_TO_STREAM  (pp, HCIC_PARAM_SIZE_IO_CAP_RYOC);
