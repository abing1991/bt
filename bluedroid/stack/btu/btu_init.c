...
static aos_task_t  btu_task_handler;
static int task_run;
aos_queue_t btu_queue;
static char queue_buf[BTU_QUEUE_LEN * sizeof(BtTaskEvt_t)];
...
void BTU_StartUp(void)
{
    ...
    int ret;
    ret = aos_queue_new(&btu_queue, queue_buf, BTU_QUEUE_LEN * sizeof(BtTaskEvt_t), sizeof(BtTaskEvt_t));
    aos_check(!ret, EINVAL);
    aos_task_new_ext(&btu_task_handler, BTU_TASK_NAME, btu_task_thread_handler, NULL, BTU_TASK_STACK_SIZE, BTU_TASK_PRIO);
    aos_check(!ret, EINVAL);
