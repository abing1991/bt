...
#define HOST_RYOCONSE                   0 //Host response when receive the BT Info Control Event
#define POLLING_RYOCONSE                1 //The BT Info response for polling by BT firmware.
...
static void rtk_handle_bt_info_control(uint8_t* p)
{
    ...
    rtk_notify_info_to_wifi(HOST_RYOCONSE, 0, NULL);
}
    ...
    switch(subcmd)
    {
        case HCI_VENDOR_SUB_CMD_BT_REPORT_CONN_SCO_INQ_INFO:
            if(status == 0) //success
            {
                ...
                rtk_notify_info_to_wifi(POLLING_RYOCONSE, (len-5), (uint8_t*)p);
            }
            break;
    ...
    if(channel_ID == 0x0001)
    {
        ...
        switch (code)
        {
            ...
            case L2CAP_CONNECTION_RSP:
                ...
                LogMsg("L2CAP_CONNECTION_RYOC, handle=%x, dcid=%x, scid=%x, result=%x", handle, dcid, scid, result);
                //if(result == 0)
