...
static aos_task_t  hcihost_task_handler;
static int task_run = 0;
static aos_queue_t hcihost_queue;
static char queue_buf[HCI_HOST_QUEUE_LEN * sizeof(BtTaskEvt_t)];
...
int hci_start_up(void)
{
    ...
    ret = aos_queue_new(&hcihost_queue, queue_buf, HCI_HOST_QUEUE_LEN * sizeof(BtTaskEvt_t), sizeof(BtTaskEvt_t));
    aos_check_return_einval(!ret);
    task_run = 1;
    ret = aos_task_new_ext(&hcihost_task_handler, HCI_HOST_TASK_NAME, hci_host_thread_handler, NULL, HCI_HOST_TASK_STACK_SIZE,  HCI_HOST_TASK_PRIO);
    aos_check_return_einval(!ret);
...
void hci_shut_down(void)
{
    ...
    aos_queue_free(&hcihost_queue);
}


task_post_status_t hci_host_task_post(SIG_HCI_HOST_t type, task_post_t timeout)
{
    BtTaskEvt_t evt;
    ...
    if (aos_queue_send(&hcihost_queue, &evt, sizeof(BtTaskEvt_t)) != 0) {
...
#if 0
    hci_host_env.recv_queue = fixed_queue_new(QUEUE_SIZE_MAX);

    if (hci_host_env.recv_queue) {
        fixed_queue_register_dequeue(hci_host_env.recv_queue, event_packet_ready);
    } else {
        HCI_TRACE_ERROR("%s unable to create pending packet queue.", __func__);
        return -1;
    }
...
static void hci_host_thread_handler(void *arg)
{
    ...
    while (task_run) {
        if (0 == aos_queue_recv(&hcihost_queue, AOS_WAIT_FOREVER, &e, &len)) {
