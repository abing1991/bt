...
#include "src/rtl8723d_fw.c"
#include "src/rtl8723d_config.c"
...
int check_match_state(bt_hw_cfg_cb_t *cfg, uint32_t mask)
{
    ...
    HCI_TRACE_DEBUG("check_match_state return %d(cfg->lmp_subversion:0x%x cfg->hci_vesion:0x%x cfg->hci_revision:0x%x cfg->chip_type:0x%x mask:%08x)\n",
...
patch_info *get_patch_entry(bt_hw_cfg_cb_t *cfg)
{
    ...
    HCI_TRACE_DEBUG("get_patch_entry(lmp_subversion:0x%x hci_vesion:0x%x cfg->hci_revision:0x%x chip_type:0x%x)\n",
    ...
    HCI_TRACE_DEBUG("get_patch_entry return(patch_name:%s config_name:%s mac_offset:0x%x)\n",
...
uint8_t line_speed_to_userial_baud(uint32_t line_speed)
{
    ...
    if (line_speed == 4000000) {
        ...
        LOG_ERROR("userial vendor: unsupported baud speed %d", line_speed);
        baud = USERIAL_BAUD_115200;
    }
...
static inline int getAltSettingVal(patch_info *patch_entry, unsigned short offset, unsigned char *val)
{
    ...
    if ((patch_entry) && (offset == patch_entry->mac_offset) && (res == 0)) {
        if (getmacaddr(val) == 0) {
            HCI_TRACE_DEBUG("MAC: %02x:%02x:%02x:%02x:%02x:%02x", val[5], val[4], val[3], val[2], val[1], val[0]);
            res = 6;
        }
    }
...
void rtk_update_altsettings(patch_info *patch_entry, unsigned char *config_buf_ptr, size_t *config_len_ptr)
{
    ...
    HCI_TRACE_DEBUG("ORG Config len=%08x:\n", config_len);

    for (i = 0; i <= config_len; i += 0x10) {
        HCI_TRACE_DEBUG("%08x: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n", i, \
    ...
    if (count <= 0) {
        HCI_TRACE_DEBUG("rtk_update_altsettings: No AltSettings");
        return;
    } else {
        HCI_TRACE_DEBUG("rtk_update_altsettings: %d AltSettings", count);
    }

    if (data_len != config_len - sizeof(struct rtk_bt_vendor_config)) {
        LOG_ERROR("rtk_update_altsettings: config len(%x) is not right(%x)", data_len, config_len - sizeof(struct rtk_bt_vendor_config));
        return;
    }
    ...
    for (i = 0; i < data_len;) {
        ...
        if (getAltSettingVal(patch_entry, le16_to_cpu(entry->offset), val) == entry->entry_len) {
            HCI_TRACE_DEBUG("rtk_update_altsettings: replace %04x[%02x]", le16_to_cpu(entry->offset), entry->entry_len);
    ...
    for (j = 0; j < count; j++) {
        ...
        memcpy(entry->entry_data, val, entry->entry_len);
        HCI_TRACE_DEBUG("rtk_update_altsettings: add %04x[%02x]", le16_to_cpu(entry->offset), entry->entry_len);
        temp = entry->entry_len + sizeof(struct rtk_bt_vendor_config_entry);
        i += temp;
        entry = (struct rtk_bt_vendor_config_entry *)((uint8_t *)entry + temp);
    }
    ...
    *config_len_ptr = i + sizeof(struct rtk_bt_vendor_config);
    ...
    HCI_TRACE_DEBUG("NEW Config len=%08x:\n", *config_len_ptr);
...
uint32_t rtk_parse_config_file(unsigned char **config_buf, size_t *filelen, uint8_t bt_addr[6], uint16_t mac_offset)
{
    ...
    if (le32_to_cpu(config->signature) != RTK_VENDOR_CONFIG_MAGIC) {
        LOG_ERROR("config signature magic number(0x%x) is not set to RTK_VENDOR_CONFIG_MAGIC", config->signature);
        return 0;
    }

    if (config_len != *filelen - sizeof(struct rtk_bt_vendor_config)) {
        LOG_ERROR("config len(0x%x) is not right(0x%x)", config_len, *filelen - sizeof(struct rtk_bt_vendor_config));
        return 0;
    }
    ...
    for (i = 0; i < config_len;) {
        switch (le16_to_cpu(entry->offset)) {
            case 0xc: {
                ...
                HCI_TRACE_DEBUG("config baud rate to :0x%08x, hwflowcontrol:0x%x, 0x%x", baudrate, entry->entry_data[12], hw_cfg_cb.hw_flow_cntrl);
                break;
            }

            default:
                HCI_TRACE_DEBUG("config offset(0x%x),length(0x%x)", entry->offset, entry->entry_len);
                break;
        }
                           ...
                           uint32_t *config_baud_rate, char *config_file_short_name, uint16_t mac_offset)
{
    ...
    if ((*config_buf = malloc(MAX_ORG_CONFIG_SIZE + MAX_ALT_CONFIG_SIZE)) == NULL) {
        LOG_ERROR("malloc buffer for config file fail(0x%x)\n", filelen);
        //close(fd);
        return -1;
    }
...
int rtk_get_bt_firmware(uint8_t **fw_buf, char *fw_short_name)
{
    ...
    if (!(*fw_buf = malloc(buf_size))) {
        LOG_ERROR("Can't alloc memory for fw&config, errno:%d", errno);
        return -1;
    }

    memcpy(*fw_buf, rtl8723d_fw, buf_size);

    HCI_TRACE_DEBUG("Load FW OK");
...
uint8_t rtk_get_fw_project_id(uint8_t *p_buf)
{
    ...
    do {
        ...
        if (opcode == 0x00) {
            if (len == 1) {
                data = *(p_buf - 2);
                HCI_TRACE_DEBUG("bt_hw_parse_project_id: opcode %d, len %d, data %d", opcode, len, data);
                break;
            } else {
                LOG_WARN("bt_hw_parse_project_id: invalid len %d", len);
            }
        }
...
struct rtk_epatch_entry *rtk_get_patch_entry(bt_hw_cfg_cb_t *cfg_cb)
{
    ...
    if (!entry) {
        LOG_ERROR("rtk_get_patch_entry: failed to allocate mem for patch entry");
        return NULL;
    }

    patch->number_of_patch = le16_to_cpu(patch->number_of_patch);

    HCI_TRACE_DEBUG("rtk_get_patch_entry: fw_ver 0x%08x, patch_num %d",
    ...
    for (i = 0; i < patch->number_of_patch; i++) {
        ...
        if (chip_id == cfg_cb->eversion + 1) {
            ...
            HCI_TRACE_DEBUG("rtk_get_patch_entry: chip_id %d, patch_len 0x%x, patch_offset 0x%x",
    ...
    if (i == patch->number_of_patch) {
        LOG_ERROR("rtk_get_patch_entry: failed to get etnry");
        free(entry);
        entry = NULL;
    }
...
void rtk_get_bt_final_patch(bt_hw_cfg_cb_t *cfg_cb)
{
    ...
    if (cfg_cb->lmp_subversion == LMPSUBVERSION_8723a) {
        if (memcmp(cfg_cb->fw_buf, RTK_EPATCH_SIGNATURE, 8) == 0) {
            LOG_ERROR("8723as check signature error!");
            cfg_cb->dl_fw_flag = 0;
            goto free_buf;
        } else {
            ...
            if (!(cfg_cb->total_buf = malloc(cfg_cb->total_len))) {
                ...
                HCI_TRACE_DEBUG("8723as, fw copy direct");
    ...
    if (memcmp(cfg_cb->fw_buf, RTK_EPATCH_SIGNATURE, 8)) {
        LOG_ERROR("check signature error");
        cfg_cb->dl_fw_flag = 0;
        goto free_buf;
    }
    ...
    if (memcmp(cfg_cb->fw_buf + cfg_cb->fw_len - 4, EXTENSION_SECTION_SIGNATURE, 4)) {
        LOG_ERROR("check extension section signature error");
        cfg_cb->dl_fw_flag = 0;
        goto free_buf;
    }
    ...
    if ((hw_cfg_cb.project_id_mask != PROJECT_ID_MASK_ALL) && ((hw_cfg_cb.project_id_mask & (1 << proj_id)) == 0)) {
        LOG_ERROR("hw_cfg_cb.project_id_mask is 0x%08x, fw project_id is %d, does not match!!!",
    ...
    HCI_TRACE_DEBUG("total_len = 0x%x", cfg_cb->total_len);

    if (!(cfg_cb->total_buf = malloc(cfg_cb->total_len))) {
        LOG_ERROR("Can't alloc memory for multi fw&config, errno:%d", errno);
        cfg_cb->dl_fw_flag = 0;
        goto free_buf;
    } else {
        ...
        HCI_TRACE_DEBUG("BTCOEX:20%06d-%04x svn_version:%d lmp_subversion:0x%x hci_version:0x%x hci_revision:0x%x chip_type:%d Cut:%d libbt-vendor_uart version:%s\n",
    ...
    cfg_cb->dl_fw_flag = 1;
    HCI_TRACE_DEBUG("Fw:%s exists, config file:%s exists", (cfg_cb->fw_len > 0) ? "" : "not", (cfg_cb->config_len > 0) ? "" : "not");
...
void hw_config_cback(void *p_mem)
{
    ...
    if (p_buf != NULL) {
        ...
        HCI_TRACE_DEBUG("hw_cfg_cb.state = %i", hw_cfg_cb.state);
        ...
        switch (hw_cfg_cb.state) {
            ...
            case HW_CFG_READ_LOCAL_VER: {
                if (status == 0) {
                    ...
                    HCI_TRACE_DEBUG("lmp_subversion = 0x%x hw_cfg_cb.hci_version = 0x%x hw_cfg_cb.hci_revision = 0x%x", hw_cfg_cb.lmp_subversion, hw_cfg_cb.hci_version, hw_cfg_cb.hci_revision);
            ...
            case HW_CFG_READ_ECO_VER: {
                if (status == 0) {
                    ...
                    HCI_TRACE_DEBUG("hw_config_cback chip_id of the IC:%d", hw_cfg_cb.eversion + 1);
                } else if (1 == status) {
                ...
                if (check_match_state(&hw_cfg_cb, 0) > 1) { // check if have multiple matched patch_entry by lmp_subversion,hci_version, hci_revision
                    ...
                    *p++ = 5;
                    ...
                    for (i = 0; i < p_buf->len; i++) {
                        HCI_TRACE_DEBUG("get chip type command data[%d]= 0x%x", i, *(pp + i));
                    }
            ...
            case HW_CFG_READ_CHIP_TYPE: {
                HCI_TRACE_DEBUG("READ_CHIP_TYPE status = %d", status);
                HCI_TRACE_DEBUG("READ_CHIP_TYPE length = %d", p_evt_buf->len);
                p = (uint8_t *)(p_evt_buf + 1) ;

                for (i = 0; i < p_evt_buf->len; i++) {
                    HCI_TRACE_DEBUG("READ_CHIP_TYPE event data[%d]= 0x%x", i, *(p + i));
                }

                if (status == 0) {
                    hw_cfg_cb.chip_type = ((*((uint8_t *)(p_evt_buf + 1) + HCI_EVT_CMD_CMPL_OPFC61_CHIPTYPE_OFFSET)) & 0x0F);
                    HCI_TRACE_DEBUG("READ_CHIP_TYPE hw_cfg_cb.lmp_subversion = 0x%x", hw_cfg_cb.lmp_subversion);
                    HCI_TRACE_DEBUG("READ_CHIP_TYPE hw_cfg_cb.hci_version = 0x%x", hw_cfg_cb.hci_version);
                    HCI_TRACE_DEBUG("READ_CHIP_TYPE hw_cfg_cb.hci_revision = 0x%x", hw_cfg_cb.hci_revision);
                    HCI_TRACE_DEBUG("READ_CHIP_TYPE hw_cfg_cb.chip_type = 0x%x", hw_cfg_cb.chip_type);
                } else {
                    is_proceeding = FALSE;
                    break;
                }
                ...
                if (check_match_state(&hw_cfg_cb, PATCH_OPTIONAL_MATCH_FLAG_CHIPTYPE) > 1) { // check if have multiple matched patch_entry by lmp_subversion,hci_version, hci_revision and chiptype
                    LOG_ERROR("check_match_state(lmp_subversion:0x%04x, hci_version:%d, hci_revision:%d chip_type:%d): Multi Matched Patch\n", hw_cfg_cb.lmp_subversion, hw_cfg_cb.hci_version, hw_cfg_cb.hci_revision, hw_cfg_cb.chip_type);
                    is_proceeding = FALSE;
                    break;
                }
...
CFG_START:
            ...
            case HW_CFG_START: {
#ifdef BT_CHIP_PROBE_SIMULATION
                {
                    ...
                    {
                        LOG_ERROR("BT_CHIP_PROBE_SIMULATION loop:%d $$$ BEGIN $$$\n", i);
                        ...
                        if (check_match_state(&hw_cfg_test, 0) > 1) {
                            LOG_ERROR("check_match_state hw_cfg_test(lmp_subversion:0x%04x, hci_version:%d, hci_revision:%d chip_type:%d): Multi Matched Patch\n", hw_cfg_test.lmp_subversion, hw_cfg_test.hci_version, hw_cfg_test.hci_revision, hw_cfg_test.chip_type);

                            if (bt_chip_chars[i].chip_type != CHIPTYPE_NONE) {
                                LOG_ERROR("BT_CHIP_PROBE_SIMULATION loop:%d *** Include ChipType ***\n", i);
                        ...
                        LOG_ERROR("BT_CHIP_PROBE_SIMULATION loop:%d $$$ END $$$\n", i);
                    }
                }
#endif
                ...
                if ((prtk_patch_file_info == NULL) || (prtk_patch_file_info->lmp_subversion == 0)) {
                    LOG_ERROR("get patch entry error");
                    is_proceeding = FALSE;
                    break;
                }
                ...
                if (hw_cfg_cb.config_len < 0) {
                    LOG_ERROR("Get Config file fail, just use efuse settings");
                ...
                if (hw_cfg_cb.fw_len < 0) {
                    LOG_ERROR("Get BT firmware fail");
                    hw_cfg_cb.fw_len = 0;
                } else {
                ...
                HCI_TRACE_DEBUG("Check total_len(0x%08x) max_patch_size(0x%08x)", hw_cfg_cb.total_len, hw_cfg_cb.max_patch_size);

                if (hw_cfg_cb.total_len > hw_cfg_cb.max_patch_size) {
                    LOG_ERROR("total length of fw&config(0x%08x) larger than max_patch_size(0x%08x)", hw_cfg_cb.total_len, hw_cfg_cb.max_patch_size);
                    is_proceeding = FALSE;
                    break;
                }
                ...
                if ((hw_cfg_cb.total_len > 0) && hw_cfg_cb.dl_fw_flag) {
                    ...
                    HCI_TRACE_DEBUG("patch fragment count %d, tail len %d", hw_cfg_cb.patch_frag_cnt, hw_cfg_cb.patch_frag_tail);
                } else {
                    is_proceeding = FALSE;
                    break;
                }
                ...
                if ((hw_cfg_cb.baudrate == 0) && ((hw_cfg_cb.hw_flow_cntrl & 0x80) == 0)) {
                    HCI_TRACE_DEBUG("no baudrate to set and no need to set hw flow control");
                    goto DOWNLOAD_FW;
                }

                if ((hw_cfg_cb.baudrate == 0) && (hw_cfg_cb.hw_flow_cntrl & 0x80)) {
                    HCI_TRACE_DEBUG("no baudrate to set but set hw flow control is needed");
                    goto SET_HW_FLCNTRL;
                }
            }
            ...
            case HW_CFG_SET_UART_BAUD_CONTROLLER:
                HCI_TRACE_DEBUG("bt vendor lib: set CONTROLLER UART baud 0x%x", hw_cfg_cb.baudrate);
            ...
            case HW_CFG_SET_UART_BAUD_HOST:
                ...
                HCI_TRACE_DEBUG("bt vendor lib: set HOST UART baud %i", host_baudrate);
...
SET_HW_FLCNTRL:
            ...
            case HW_CFG_SET_UART_HW_FLOW_CONTROL:
                HCI_TRACE_DEBUG("Change HW flowcontrol setting");
...
DOWNLOAD_FW:
            ...
            case HW_CFG_DL_FW_PATCH:
                HCI_TRACE_DEBUG("bt vendor lib: HW_CFG_DL_FW_PATCH status:%i, opcode:0x%x", status, opcode);
                ...
                if (opcode == HCI_VSC_DOWNLOAD_FW_PATCH) {
                    ...
                    HCI_TRACE_DEBUG("bt vendor lib: HW_CFG_DL_FW_PATCH status:%i, iIndexRx:%i", status, iIndexRx);
                    hw_cfg_cb.patch_frag_idx++;

                    if (iIndexRx & 0x80) {
                        HCI_TRACE_DEBUG("vendor lib fwcfg completed");
                ...
                if (hw_cfg_cb.patch_frag_idx < hw_cfg_cb.patch_frag_cnt) {
                    ...
                    if (hw_cfg_cb.patch_frag_idx == hw_cfg_cb.patch_frag_cnt - 1) {
                        HCI_TRACE_DEBUG("HW_CFG_DL_FW_PATCH: send last fw fragment");
                        iIndexRx |= 0x80;
    ...
    if (is_proceeding == FALSE) {
        LOG_ERROR("vendor lib fwcfg aborted!!!");
...
void hw_config_start(void)
{
    ...
    HCI_TRACE_DEBUG("RTKBT_RELEASE_NAME: %s", RTKBT_RELEASE_NAME);
    HCI_TRACE_DEBUG("\nRealtek libbt-vendor_uart Version %s \n", RTK_VERSION);
    HC_BT_HDR  *p_buf = NULL;
    uint8_t     *p;

    HCI_TRACE_DEBUG("hw_config_start\n");
...
void hw_epilog_process(void)
{
    ...
    if (p_buf) {
        ...
        *p = 0; /* parameter length */
        ...
        if (bt_vendor_cbacks) {
            BTHWDBG("vendor lib epilog process aborted [no buffer]");
