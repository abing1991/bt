...
**  Constants & Macros
******************************************************************************/
//if you want to see data tx and rx, set H5_TRACE_DATA_ENABLE 1
...
static sk_buff *h5_dequeue()
{
    ...
    //    HCI_TRACE_DEBUG("RtbGetQueueLen(rtk_h5.unack) = (%d), sliding_window_size = (%d)", RtbGetQueueLen(rtk_h5.unack), rtk_h5.sliding_window_size);
...
static uint16_t h5_wake_up()
{
    ...
    while (NULL != (skb = h5_dequeue())) {
        ...
        bytes_sent =  uart_send(uart_dev, data, data_len);

        //HCI_TRACE_DEBUG("bytes_sent(%d)", bytes_sent);

#if H5_TRACE_DATA_ENABLE
        {
            uint32_t iTemp = 0;
            uint32_t iTempTotal = 16;
            HCI_TRACE_DEBUG("H5 TX: length(%d)", data_len);
            ...
            for (iTemp = 0; iTemp < iTempTotal; iTemp++) {
                HCI_TRACE_DEBUG("0x%x", data[iTemp]);
            }
        }
#endif
        skb_free(&skb);
...
void h5_process_ctl_pkts(void)
{
    ...
    uint32_t len = skb_get_data_length(skb);
    printf("++++++recv packet:\n");
            ...
            if (opcode == p_cb->int_cmd[p_cb->int_cmd_rd_idx].opcode) {
                ...
                if (opcode == HCI_VSC_UPDATE_BAUDRATE) {
                    ...
                    if (p_cb->int_cmd[p_cb->int_cmd_rd_idx].cback != NULL) {
                        ...
                        HCI_TRACE_DEBUG("CommandCompleteEvent for command Missing cback function buffer_allocator->free(p_cb->p_rcv_msg) (0x%04X)", opcode);
...
static uint8_t hci_recv_frame(sk_buff *skb, uint8_t pkt_type)
{
...
#if H5_TRACE_DATA_ENABLE
    {
        uint32_t iTemp = 0;
        uint32_t iTempTotal = 16;
        HCI_TRACE_DEBUG("H5 RX: length(%d)", data_len);
...
static uint8_t h5_complete_rx_pkt(tHCI_H5_CB *h5)
{
    ...
    //HCI_TRACE_DEBUG("HCI 3wire h5_complete_rx_pkt");
    h5_hdr = (uint8_t *)skb_get_data(h5->rx_skb);
    HCI_TRACE_DEBUG("SeqNumber(%d), AckNumber(%d)", H5_HDR_SEQ(h5_hdr), H5_HDR_ACK(h5_hdr));
    ...
    if (H5_HDR_RELIABLE(h5_hdr)) {
        //HCI_TRACE_DEBUG("Received reliable seqno %u from card", h5->rxseq_txack);
        osi_mutex_lock(&h5_wakeup_mutex, OSI_MUTEX_MAX_TIMEOUT);
...
static uint32_t h5_recv(tHCI_H5_CB *h5, uint8_t *data, int count)
{
    ...
    //HCI_TRACE_ERROR("count %d rx_state %d rx_count %ld", count, h5->rx_state, h5->rx_count);
    ...
    while (count) {
        if (h5->rx_count) {
            ...
            //HCI_TRACE_ERROR("rx_count: %d  conut: %d", h5->rx_count, count);
        ...
        switch (h5->rx_state) {
            case H5_W4_HDR:
                ...
                if (H5_HDR_RELIABLE(hdr)
                    ...
                    HCI_TRACE_ERROR("Out-of-order packet arrived, got(%u)expected(%u)",
                                    H5_HDR_SEQ(hdr), h5->rxseq_txack);
                    h5->is_txack_req = 1;
                    h5_wake_up();
...
static void data_ready_cb_thread(void *arg)
{
    ...
    while (h5_data_ready_running) {
        ...
        int length = skb_get_data_length(rtk_h5.data_skb);

        BT_HDR *packet = rtk_h5.allocator->alloc(length + sizeof(BT_HDR));
        ...
        memcpy(packet->data, skb_get_data(rtk_h5.data_skb), length);

        packet->layer_specific = 0;
        packet->offset = 0;
        packet->len = length - packet->offset;
        packet->event = outbound_event_types[PACKET_TYPE_TO_INDEX(skb_get_pkt_type(rtk_h5.data_skb))];
...
static void data_retransfer_thread(void *arg)
{
    ...
    while (h5_retransfer_running) {
        ...
        if (events & H5_EVENT_RX) {
            sk_buff *skb;
            HCI_TRACE_ERROR("retransmitting (%u) pkts, retransfer count(%d)", skb_queue_get_length(rtk_h5.unack), rtk_h5.data_retrans_count);
...
static int create_data_retransfer_thread()
{
    ...
    if (aos_task_new_ext(&rtk_h5.thread_data_retrans, "data_retransfer",
                         (void *)data_retransfer_thread, NULL, 3072, AOS_DEFAULT_APP_PRI) != 0) {
...
static int create_data_ready_cb_thread()
{
    ...
    if (aos_task_new_ext(&rtk_h5.thread_data_ready_cb, "data_ready_cb",
                         (void *)data_ready_cb_thread, NULL, 3072, AOS_DEFAULT_APP_PRI) != 0) {
...
void hci_h5_int_init(hci_h5_hal_callbacks_t *h5_callbacks)
{
    ...
    rtk_h5.allocator = buffer_allocator_get_interface();
    ...
    static uint8_t local_bd_addr[6] = {0x01, 0x02, 0x00, 0x00, 0x00, 0x00};
#ifdef BLUETOOTH_RTK
    h5_vendor->open(local_bd_addr, hci_layer_get_interface());

    h5_vendor->set_callback(VENDOR_CONFIGURE_FIRMWARE, firmware_config_callback);
    ...
    h5_vendor->send_command(VENDOR_CHIP_POWER_CONTROL, &power_state);

    power_state = BT_VND_PWR_ON;
    h5_vendor->send_command(VENDOR_CHIP_POWER_CONTROL, &power_state);
#endif

    ret = h5_vendor->send_command(VENDOR_OPEN_USERIAL, &uart_dev);

    aos_check(!ret, EIO);

#ifdef BLUETOOTH_RTK
    ret = osi_sem_new(&startup, 0, 0);

    aos_check(!ret, EIO);

    h5_vendor->send_async_command(VENDOR_CONFIGURE_FIRMWARE, NULL);

    osi_sem_take(&startup, OSI_SEM_MAX_TIMEOUT);

    osi_sem_free(&startup);
#endif
}
...
{
    uint32_t read_len = uart_recv(uart_dev, data_buffer, sizeof(data_buffer), 0);
...
static int OsStartTimer(osi_alarm_t *timerid, int msec, int mode)
{
    ...
    if (osi_alarm_set(timerid, msec) != 0) {
