...
yoc_err_t yoc_spp_init(yoc_spp_mode_t mode)
{
    ...
    arg.init.mode = mode;
    return (btc_transfer_context(&msg, &arg, sizeof(btc_spp_args_t), NULL) == BT_STATUS_SUCCESS ? YOC_OK : YOC_FAIL);
}
...
yoc_err_t yoc_spp_start_discovery(yoc_bd_addr_t bd_addr)
{
    ...
    memcpy(arg.start_discovery.bd_addr, bd_addr, YOC_BD_ADDR_LEN);
    arg.start_discovery.num_uuid = 1;
    arg.start_discovery.p_uuid_list = &sdp_uuid;

    return (btc_transfer_context(&msg, &arg, sizeof(btc_spp_args_t), btc_spp_arg_deep_copy) == BT_STATUS_SUCCESS ? YOC_OK : YOC_FAIL);
}
                          ...
                          yoc_spp_role_t role, uint8_t remote_scn, yoc_bd_addr_t peer_bd_addr)
{
    ...
    memcpy(arg.connect.peer_bd_addr, peer_bd_addr, YOC_BD_ADDR_LEN);
...
yoc_err_t yoc_spp_vfs_register()
{
    YOC_BLUEDROID_STATUS_CHECK(YOC_BLUEDROID_STATUS_ENABLED);
