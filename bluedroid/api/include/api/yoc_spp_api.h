...
typedef enum {
    ...
    YOC_SPP_DATA_IND_EVT                = 30,               /*!< When SPP connection received data, the event comes, olny for YOC_SPP_MODE_CB */
    YOC_SPP_CONG_EVT                    = 31,               /*!< When SPP connection congestion status changed, the event comes, olny for YOC_SPP_MODE_CB */
    YOC_SPP_WRITE_EVT                   = 33,               /*!< When SPP write operation completes, the event comes, olny for YOC_SPP_MODE_CB */
...
typedef union {
    ...
    struct spp_open_evt_param {
        ...
        int                 fd;             /*!< The file descriptor olny for YOC_SPP_MODE_VFS*/
 ...
 *              - other: failed
 */
yoc_err_t yoc_spp_register_callback(yoc_spp_cb_t callback);
 ...
 *              - other: failed
 */
yoc_err_t yoc_spp_init(yoc_spp_mode_t mode);
 ...
 *              - other: failed
 */
yoc_err_t yoc_spp_start_discovery(yoc_bd_addr_t bd_addr);
 ...
 *              - other: failed
 */
yoc_err_t yoc_spp_connect(yoc_spp_sec_t sec_mask,
 ...
 *              - other: failed
 */
yoc_err_t yoc_spp_disconnect(uint32_t handle);
 ...
 *              - other: failed
 */
yoc_err_t yoc_spp_start_srv(yoc_spp_sec_t sec_mask,
                            yoc_spp_role_t role, uint8_t local_scn, const char *name);


/**
 * @brief       This function is used to write data, olny for YOC_SPP_MODE_CB.
 *
 * @param[in]   handle: The connection handle.
 ...
 *              - other: failed
 */
yoc_err_t yoc_spp_write(uint32_t handle, int len, uint8_t *p_data);
 ...
 *              - other: failed
 */
yoc_err_t yoc_spp_vfs_register(void);
